...
/*!50001 VIEW `choose_lens` AS select `LENS`.`lens_id` AS `id`,concat(`MANUFACTURER`.`manufacturer`,' ',`LENSMODEL`.`model`) AS `opt` from ((`LENS` join `LENSMODEL` on(`LENS`.`lensmodel_id` = `LENSMODEL`.`lensmodel_id`)) join `MANUFACTURER` on(`LENSMODEL`.`manufacturer_id` = `MANUFACTURER`.`manufacturer_id`)) where `LENS`.`own` = 1 and `LENSMODEL`.`fixed_mount` = 0 order by concat(`MANUFACTURER`.`manufacturer`,' ',`LENSMODEL`.`model`) */;
