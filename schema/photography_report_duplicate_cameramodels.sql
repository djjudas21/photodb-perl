...
/*!50001 VIEW `report_duplicate_cameramodels` AS select `CAMERAMODEL`.`cameramodel_id` AS `Camera Model ID`,concat(`MANUFACTURER`.`manufacturer`,' ',`CAMERAMODEL`.`model`) AS `Camera`,`MOUNT`.`mount` AS `Mount`,`LENSMODEL`.`model` AS `Lens`,`FORMAT`.`format` AS `Format`,`CAMERAMODEL`.`introduced` AS `Introduced`,`CAMERAMODEL`.`notes` AS `Notes` from (((((`CAMERAMODEL` join `MANUFACTURER` on(`CAMERAMODEL`.`manufacturer_id` = `MANUFACTURER`.`manufacturer_id`)) left join `MOUNT` on(`CAMERAMODEL`.`mount_id` = `MOUNT`.`mount_id`)) left join `FORMAT` on(`CAMERAMODEL`.`format_id` = `FORMAT`.`format_id`)) left join `LENS` on(`CAMERAMODEL`.`lensmodel_id` = `LENS`.`lens_id`)) left join `LENSMODEL` on(`LENS`.`lensmodel_id` = `LENSMODEL`.`lensmodel_id`)) where `CAMERAMODEL`.`model` in (select `CAMERAMODEL`.`model` from `CAMERAMODEL` group by `CAMERAMODEL`.`model` having count(`CAMERAMODEL`.`model`) > 1) order by `CAMERAMODEL`.`model` */;
