...
sub film_add {
	...
	if ($href->{bulk_loaded} // &prompt({default=>'no', prompt=>'Is this film bulk-loaded?', type=>'boolean'}) == 1) {
		...
		$data{film_bulk_id} = $href->{film_bulk_id} // &listchoices({table=>'choose_bulk_film', required=>1});
		...
		$data{film_batch} = $href->{film_batch} // &lookupval({col=>'batch', table=>'FILM_BULK', where=>{film_bulk_id=>$data{'film_bulk_id'}}});
		$data{film_expiry} = $href->{film_expiry} // &lookupval({col=>'expiry', table=>'FILM_BULK', where=>{film_bulk_id=>$data{'film_bulk_id'}}});
		$data{purchase_date} = $href->{purchase_date} // &lookupval({col=>'purchase_date', table=>'FILM_BULK', where=>{film_bulk_id=>$data{'film_bulk_id'}}});
		$data{filmstock_id} = $href->{filmstock_id} // &lookupval({col=>'filmstock_id', table=>'FILM_BULK', where=>{film_bulk_id=>$data{'film_bulk_id'}}});
		$data{format_id} = $href->{format_id} // &lookupval({col=>'format_id', table=>'FILM_BULK', where=>{film_bulk_id=>$data{'film_bulk_id'}}});
	} else {
		...
		$data{filmstock_id} = $href->{filmstock_id} // &listchoices({table=>'choose_filmstock', inserthandler=>\&filmstock_add, required=>1});
		$data{format_id} = $href->{format_id} // &listchoices({cols=>['format_id as id', 'format as opt'], table=>'FORMAT', inserthandler=>\&format_add}, required=>1);
	}
...
sub film_load {
	my $href = shift;
	my $film_id = $href->{film_id} // &listchoices({table=>'choose_film_to_load', required=>1});
	my %data;
	$data{camera_id} = $href->{camera_id} // &listchoices({table=>'choose_camera_by_film', where=>{film_id=>$film_id}, required=>1});
	$data{exposed_at} = $href->{exposed_at} // &prompt({default=>&lookupval({col=>"iso", table=>'FILM join FILMSTOCK on FILM.filmstock_id=FILMSTOCK.filmstock_id', where=>{film_id=>$film_id}}), prompt=>'What ISO?', type=>'integer'});
	...
	return &updaterecord({data=>\%data, table=>'FILM', where=>{film_id=>$film_id}});
}

# Put a film in a physical archive
sub film_archive {
	my $href = shift;
	my $film_id = $href->{film_id} // &film_choose;
	my %data;
	$data{archive_id} = $href->{archive_id} // &listchoices({table=>'ARCHIVE', cols=>['archive_id as id', 'name as opt'], where=>['archive_type_id in (1,2)', 'sealed = 0'], inserthandler=>\&archive_add, required=>1});
	return &updaterecord({data=>\%data, table=>'FILM', where=>{film_id=>$film_id}});
}
...
sub film_develop {
	my $href = shift;
	my $film_id = $href->{film_id} // &listchoices({table=>'choose_film_to_develop', required=>1});
	my %data;
	$data{date} = $href->{date} // &prompt({default=>&today, prompt=>'What date was this film processed?', type=>'date'});
	$data{developer_id} = $href->{developer_id} // &listchoices({table=>'DEVELOPER', cols=>['developer_id as id', 'name as opt'], where=>{'for_film'=>1}, inserthandler=>\&developer_add});
	...
	&updaterecord({data=>\%data, table=>'FILM', where=>{film_id=>$film_id}});
	if (&prompt({default=>'no', prompt=>'Archive this film now?', type=>'boolean'})) {
		&film_archive({film_id=>$film_id});
	}
	return;
}
...
sub film_info {
	my $href = shift;
	my $film_id = $href->{film_id} // &film_choose;
	print Dump(&lookupcol({table=>'info_film', where=>{'`Film ID`'=>$film_id}}));
	return;
}
...
sub film_locate {
	...
	if (my $archiveid = &lookupval({col=>'archive_id', table=>'FILM', where=>{film_id=>$film_id}})) {
		my $archive = &lookupval({col=>"concat(name, ' (', location, ')') as archive", table=>'ARCHIVE', where=>{archive_id=> $archiveid}});
...
sub film_bulk {
	my $href = shift;
	my %data;
	$data{filmstock_id} = &listchoices({table=>'choose_filmstock', inserthandler=>\&filmstock_add, required=>1});
	$data{format_id} = &listchoices({cols=>['format_id as id', 'format as opt'], table=>'FORMAT', inserthandler=>\&format_add, required=>1});
	...
	return &newrecord({data=>\%data, table=>'FILM_BULK'});
}
...
sub film_stocks {
	...
	my $rows = &tabulate({view=>'view_film_stocks'});
...
sub film_current {
	my $href = shift;
	&printlist({msg=>"current films", table=>'current_films'});
	return;
}
	...
	while (1) {
		...
		if ($film_id ne '') {
			...
			if (&prompt({default=>'no', prompt=>'Narrow search by film format?', type=>'boolean'})) {
				$where{format_id} = &listchoices({cols=>['format_id as id', 'format as opt'], table=>'FORMAT'});
			}
			if (&prompt({default=>'no', prompt=>'Narrow search by filmstock?', type=>'boolean'})) {
				$where{filmstock_id} = &listchoices({table=>'choose_filmstock'});
			}
			if (&prompt({default=>'no', prompt=>'Narrow search by the camera the film was loaded into?', type=>'boolean'})) {
				$where{camera_id} = &listchoices({table=>'choose_camera'});
			}
			#listchoices
			$where{notes} = {'!=',undef};
			my $thinwhere = &thin(\%where);
			return &listchoices({cols=>['film_id as id', 'notes as opt'], table=>'FILM', where=>$thinwhere, required=>1});
		}
	}
	return;
}
...
sub cameramodel_add {
	...
	my $datahr = &camera_prompt;
	my %data = %$datahr;

	# Insert new record into DB
	my $cameramodel_id = &newrecord({data=>\%data, table=>'CAMERAMODEL'});
...
sub cameramodel_info {
        ...
        # Choose camera
        my $cameramodel_id = $href->{cameramodel_id} // &listchoices({table=>'choose_cameramodel', required=>1});

        # Get camera data
        my $cameradata = &lookupcol({table=>'info_cameramodel', where=>{'`Camera Model ID`'=>$cameramodel_id}});

        # Show compatible accessories
        my $accessories = &lookuplist({col=>'opt', table=>'choose_accessory_compat', where=>{cameramodel_id=>$cameramodel_id}});
...
sub camera_add {
	...
	if ($manufacturer_id == 3 && $data{datecode}) {
		my $introduced = &lookupval({col=>'introduced', table=>'CAMERAMODEL', where=>{cameramodel_id=>$data{cameramodel_id}}});
		my $discontinued = &lookupval({col=>'discontinued', table=>'CAMERAMODEL', where=>{cameramodel_id=>$data{cameramodel_id}}});
	...
	if (&lookupval({col=>'fixed_mount', table=>'CAMERAMODEL', where=>{cameramodel_id=>$data{cameramodel_id}}})) {
		...
		my $lensmodel_id = &lookupval({col=>'lensmodel_id', table=>'CAMERAMODEL', where=>{cameramodel_id=>$data{cameramodel_id}}});

		# Get info about lens
		print "Please enter some information about the lens\n";
		$data{lens_id} = &lens_add({cost=>0, own=>$data{own}, lensmodel_id=>$lensmodel_id, acquired=>$data{acquired}, source=>$data{source}});
	}

	if (defined($data{mount_id})) {
		$data{display_lens} = &listchoices({table=>'choose_display_lens', where=>{mount_id=>$data{mount_id}}, skipok=>1});
	}
	# Insert new record into DB
	my $camera_id = &newrecord({data=>\%data, table=>'CAMERA'});
...
sub camera_edit {
	my $href = shift;
	my $camera_id = $href->{camera_id} // &listchoices({table=>'choose_camera', required=>1});
	...
	$data{condition_id} = &listchoices({keyword=>'condition', cols=>['condition_id as id', 'name as opt'], table=>'`CONDITION`', default=>$$existing{condition_id}});
	$data{condition} = &prompt({prompt=>'Condition description'});
	...
	# Update the DB
	return &updaterecord({data=>$changes, table=>'CAMERA', where=>{camera_id=>$camera_id}});
}
...
sub camera_prompt {
	...
	my %data;
	$data{manufacturer_id} = &choose_manufacturer({default=>$$defaults{manufacturer_id}});
	...
	if (defined($data{fixed_mount}) && $data{fixed_mount} == 1 && !defined($$defaults{lens_id})) {
		...
		$data{lensmodel_id} = &listchoices({table=>'choose_lensmodel', required=>1, inserthandler=>\&lensmodel_add});
	} else {
		$data{mount_id} = &listchoices({cols=>['mount_id as id', 'mount as opt'], table=>'choose_mount', where=>{purpose=>'Camera'}, inserthandler=>\&mount_add, default=>$$defaults{mount_id}});
	}
	$data{format_id} = &listchoices({cols=>['format_id as id', 'format as opt'], table=>'FORMAT', inserthandler=>\&format_add, required=>1, default=>$$defaults{format_id}});
	$data{negative_size_id} = &listchoices({cols=>['negative_size_id as id', 'negative_size as opt'], table=>'choose_negativeformat', where=>{format_id=>$data{format_id}}, inserthandler=>\&negative_size_compat, default=>$$defaults{negative_size_id}});
	$data{focus_type_id} = &listchoices({cols=>['focus_type_id as id', 'focus_type as opt'], table=>'FOCUS_TYPE', inserthandler=>\&focustype_add, default=>$$defaults{focus_type_id}});
	...
	if (defined($data{metering}) && $data{metering} == 1) {
		...
		$data{metering_type_id} = &listchoices({cols=>['metering_type_id as id', 'metering as opt'], table=>'METERING_TYPE', inserthandler=>\&meteringtype_add, default=>$$defaults{metering_type_id}});
	...
	$data{body_type_id} = &listchoices({cols=>['body_type_id as id', 'body_type as opt'], table=>'BODY_TYPE', inserthandler=>\&camera_addbodytype, default=>$$defaults{body_type_id}});
	...
	$data{shutter_type_id} = &listchoices({cols=>['shutter_type_id as id', 'shutter_type as opt'], table=>'SHUTTER_TYPE', inserthandler=>\&shuttertype_add, default=>$$defaults{shutter_type_id}});
	...
	if (defined($data{battery_qty}) && $data{battery_qty} > 0) {
		$data{battery_type} = &listchoices({keyword=>'battery type', table=>'choose_battery', inserthandler=>\&battery_add, default=>$$defaults{battery_type}});
	}
	...
	if ($data{int_flash} == 1 || $data{ext_flash} == 1) {
		...
		$data{flash_metering} = &listchoices({table=>'choose_flash_protocol', inserthandler=>\&flashprotocol_add, default=>$$defaults{flash_metering}});
	}
...
sub cameramodel_accessory {
	...
	while (1) {
		my %compatdata;
		$compatdata{accessory_id} = &listchoices({table=>'choose_accessory'});
		$compatdata{cameramodel_id} = $cameramodel_id;
		&newrecord({data=>\%compatdata, table=>'ACCESSORY_COMPAT', silent=>1});
...
sub negative_size_compat {
	my $href = shift;
	my %compatdata;
	$compatdata{format_id} = $href->{format_id} // &listchoices({cols=>['format_id as id', 'format as opt'], table=>'FORMAT'});
	$compatdata{negative_size_id} = &listchoices({cols=>['negative_size_id as id', 'negative_size as opt'], table=>'NEGATIVE_SIZE', inserthandler=>\&negativesize_add});
	&newrecord({data=>\%compatdata, table=>'NEGATIVEFORMAT_COMPAT', silent=>1});
...
sub format_compat {
        my $href = shift;
        my %compatdata;
        $compatdata{negative_size_id} = $href->{negative_size_id} // &listchoices({cols=>['negative_size_id as id', 'negative_size as opt'], table=>'NEGATIVE_SIZE'});
        $compatdata{format_id} = &listchoices({cols=>['format_id as id', 'format as opt'], table=>'FORMAT', inserthandler=>\&format_add});
        &newrecord({data=>\%compatdata, table=>'NEGATIVEFORMAT_COMPAT', silent=>1});
...
sub cameramodel_exposureprogram {
	my $href = shift;
	my $cameramodel_id = $href->{cameramodel_id} // &listchoices({table=>'choose_cameramodel', required=>1});
	my $exposureprograms = &lookupcol({table=>'EXPOSURE_PROGRAM'});
	foreach my $exposureprogram (@$exposureprograms) {
		...
		if (&prompt({default=>'no', prompt=>"Does this camera have $exposureprogram->{exposure_program} exposure program?", type=>'boolean'})) {
			...
			&newrecord({data=>\%epdata, table=>'EXPOSURE_PROGRAM_AVAILABLE', silent=>1});
...
sub cameramodel_meteringmode {
	my $href = shift;
	my $cameramodel_id = $href->{cameramodel_id} // &listchoices({table=>'choose_cameramodel', required=>1});
	my $meteringmodes = &lookupcol({table=>'METERING_MODE'});
	foreach my $meteringmode (@$meteringmodes) {
		if (&prompt({default=>'no', prompt=>"Does this camera have $meteringmode->{metering_mode} metering?", type=>'boolean'})) {
			...
			&newrecord({data=>\%mmdata, table=>'METERING_MODE_AVAILABLE', silent=>1});
...
sub cameramodel_series {
	my $href = shift;
	my %data;
	$data{cameramodel_id} = $href->{cameramodel_id} // &listchoices({table=>'choose_cameramodel', required=>1});
	$data{series_id} = $href->{series_id} // &listchoices({cols=>['series_id as id', 'name as opt'], table=>'SERIES', required=>1, inserthandler=>\&series_add});
	return &newrecord({data=>\%data, table=>'SERIES_MEMBER'});
}
...
sub camera_displaylens {
	my $href = shift;
	my %data;
	my $camera_id = $href->{camera_id} // &listchoices({keyword=>'camera', table=>'choose_camera', where=>{mount_id=>{'!=', undef}}, required=>1 });
	my $mount = &lookupval({col=>'mount_id', table=>'choose_camera', where=>{id=>$camera_id}});
	$data{display_lens} = &listchoices({table=>'choose_display_lens', where=>{camera_id=>[$camera_id, undef], mount_id=>$mount}, default=>&lookupval({col=>'display_lens', table=>'CAMERA', where=>{camera_id=>$camera_id}})});
	return &updaterecord({data=>\%data, table=>'CAMERA', where=>{camera_id=>$camera_id}});
}
...
sub camera_sell {
	my $href = shift;
	my $camera_id = $href->{camera_id} // &listchoices({table=>'choose_camera'});
	my %data;
	$data{own} = 0;
	...
	&updaterecord({data=>\%data, table=>'CAMERA', where=>{camera_id=>$camera_id}});
	&unsetdisplaylens({camera_id=>$camera_id});
	if (my $lens_id = &lookupval({col=>'lens_id', table=>'CAMERA', where=>{camera_id=>$camera_id}})) {
		my %lensdata;
		$lensdata{own} = 0;
		$lensdata{lost} = $data{lost};
		$lensdata{lost_price} = 0;
		&updaterecord({data=>\%lensdata, table=>'LENS', where=>{lens_id=>$lens_id}});
	}
	return;
}
...
sub camera_repair {
	my $href = shift;
	my %data;
	$data{camera_id} = $href->{camera_id} // &listchoices({table=>'choose_camera'});
	...
	return &newrecord({data=>\%data, table=>'REPAIR'});
}
...
sub camera_info {
	...
	# Get camera data
	my $cameradata = &lookupcol({table=>'info_camera', where=>{'`Camera ID`'=>$camera_id}});
	...
	my $lenses = &lookuplist({col=>'lens', table=>'cameralens_compat', where=>{camera_id=>$camera_id}});
...
sub camera_choose {
	my $href = shift;
	my %where;
	$where{manufacturer_id} = $href->{manufacturer_id} // &choose_manufacturer;
	$where{format_id} = $href->{format_id} // &listchoices({cols=>['format_id as id', 'format as opt'], table=>'FORMAT'});
	...
	if ($where{fixed_mount} && $where{fixed_mount} != 1) {
		$where{mount_id} = $href->{mount_id} // &listchoices({cols=>['mount_id as id', 'mount as opt'], table=>'choose_mount', where=>{'purpose'=>'Camera'}});
	}
	$where{focus_type_id} = $href->{focus_type_id} // &listchoices({cols=>['focus_type_id as id', 'focus_type as opt'], table=>'FOCUS_TYPE', 'integer'});
	...
	if ($where{metering} && $where{metering} == 1) {
		...
		$where{metering_type_id} = $href->{metering_type_id} // &listchoices({cols=>['metering_type_id as id', 'metering as opt'], table=>'METERING_TYPE'});
	}
	$where{body_type_id} = $href->{body_type_id} // &listchoices({cols=>['body_type_id as id', 'body_type as opt'], table=>'BODY_TYPE'});
	$where{negative_size_id} = $href->{negative_size_id} // &listchoices({cols=>['negative_size_id as id', 'negative_size as opt'], table=>'NEGATIVE_SIZE'});
	...
	if (($where{int_flash} && $where{int_flash} == 1) || ($where{ext_flash} && $where{ext_flash} == 1)) {
		...
		$where{flash_metering} = $href->{flash_metering} // &listchoices({table=>'choose_flash_protocol'});
	}
	...
	my $thinwhere = &thin(\%where);
	&printlist({msg=>"cameras that match your criteria", table=>'camera_chooser', where=>$thinwhere});
	return;
}
...
sub negative_add {
	my $href = shift;
	my %data;
	$data{film_id} = $href->{film_id} // &film_choose;
	if (!&lookupval({col=>'camera_id', table=>'FILM', where=>{film_id=>$data{film_id}}})) {
		...
		if (&prompt({default=>'yes', prompt=>'Load film into a camera now?', type=>'boolean'})) {
			&film_load({film_id=>$data{film_id}});
		} else {
			return;
		}
	}
	...
	$data{lens_id} = $href->{lens_id} // &listchoices({keyword=>'lens', table=>'choose_lens_by_film', where=>{film_id=>$data{film_id}}});
	$data{shutter_speed} = $href->{shutter_speed} // &choose_shutterspeed({film_id=>$data{film_id}});
	$data{aperture} = $href->{aperture} // &prompt({prompt=>'Aperture', type=>'decimal'});
	my $filter_dia = 0;
	if ($data{lens_id}) {
		$filter_dia = &lookupval({col=>'if(filter_thread, filter_thread, 0)', table=>'LENS join LENSMODEL on LENS.lensmodel_id=LENSMODEL.lensmodel_id', where=>{lens_id=>$data{lens_id}}});
	}
	$data{filter_id} = $href->{filter_id} // &listchoices({table=>'choose_filter', where=>{'thread'=>{'>=', $filter_dia}}, inserthandler=>\&filter_add, skipok=>1, autodefault=>0});
	$data{teleconverter_id} = $href->{teleconverter_id} // &listchoices({keyword=>'teleconverter', table=>'choose_teleconverter_by_film', where=>{film_id=>$data{film_id}}, inserthandler=>\&teleconverter_add, skipok=>1, autodefault=>0});
	$data{notes} = $href->{notes} // &prompt({prompt=>'Extra notes'});
	$data{mount_adapter_id} = $href->{mount_adapter_id} // &listchoices({table=>'choose_mount_adapter_by_film', where=>{film_id=>$data{film_id}}, skipok=>1});
	$data{focal_length} = $href->{focal_length} // &prompt({default=>&lookupval({col=>'min_focal_length', table=>'LENS', where=>{lens_id=>$data{'lens_id'}}}), prompt=>'Focal length', type=>'integer'});
	...
	$data{metering_mode} = $href->{metering_mode} // &listchoices({cols=>['metering_mode_id as id', 'metering_mode as opt'], table=>'METERING_MODE'});
	$data{exposure_program} = $href->{exposure_program} // &listchoices({cols=>['exposure_program_id as id', 'exposure_program as opt'], table=>'EXPOSURE_PROGRAM'});
	$data{photographer_id} = $href->{photographer_id} // &listchoices({keyword=>'photographer', cols=>['person_id as id', 'name as opt'], table=>'PERSON', inserthandler=>\&person_add});
	if (&prompt({prompt=>'Is this negative duplicated from another?', type=>'boolean', default=>'no'})) {
		$data{copy_of} = $href->{copy_of} // &chooseneg({oktoreturnundef=>1});
	}
	return &newrecord({data=>\%data, table=>'NEGATIVE'});
}
...
sub negative_bulkadd {
	...
	if (&prompt({default=>'no', prompt=>"Add any other attributes to all $num negatives?", type=>'boolean'})) {
		$data{description} = $href->{description} // &prompt({prompt=>'Caption'});
		$data{date} = $href->{date} // &prompt({default=>&today, prompt=>'What date was this negative taken?', type=>'date'});
		$data{lens_id} = $href->{lens_id} // &listchoices({keyword=>'lens', table=>'choose_lens_by_film', where=>{film_id=>$data{film_id}}, skipok=>1});
		$data{shutter_speed} = $href->{shutter_speed} // &choose_shutterspeed({film_id=>$data{film_id}});
		$data{aperture} = $href->{aperture} // &prompt({prompt=>'Aperture', type=>'decimal'});
		$data{filter_id} = $href->{filter_id} // &listchoices({table=>'choose_filter', inserthandler=>\&filter_add, skipok=>1, autodefault=>0});
		$data{teleconverter_id} = $href->{teleconverter_id} // &listchoices({keyword=>'teleconverter', table=>'choose_teleconverter_by_film', where=>{film_id=>$data{film_id}}, inserthandler=>\&teleconverter_add, skipok=>1, autodefault=>0});
		$data{notes} = $href->{notes} // &prompt({prompt=>'Extra notes'});
		$data{mount_adapter_id} = $href->{mount_adapter_id} // &listchoices({table=>'choose_mount_adapter_by_film', where=>{film_id=>$data{film_id}}, skipok=>1});
		$data{focal_length} = $href->{focal_length} // &prompt({default=>&lookupval({col=>'min_focal_length', table=>'LENS', where=>{lens_id=>$data{lens_id}}}), prompt=>'Focal length', type=>'integer'});
	...
	for my $i (1..$num) {
		...
		# Create a new row
		&newrecord({data=>\%$thindata, table=>'NEGATIVE', silent=>1});
	}
...
sub negative_info {
	my $href = shift;
	my $negative_id = $href->{negative_id} // &chooseneg;
	print Dump(&lookupcol({table=>'info_negative', where=>{'`Negative ID`'=>$negative_id}}));
	return;
}
...
sub negative_prints {
	my $href = shift;
	my $neg_id = &chooseneg;
	&printlist({msg=>"prints from negative $neg_id", cols=>'Print as id, concat(Size, \' - \', Location) as opt', table=>'info_print', where=>{'`Negative ID`'=>$neg_id}});
	return;
}
...
sub negative_tag {
	my $href = shift;
	my $neg_id = $href->{negative_id} // &chooseneg;
	&tag({where=>{negative_id=>$neg_id}});
	return;
}
...
sub lens_add {
	...
	my %data;
	my $manufacturer_id = $href->{manufacturer_id} // &choose_manufacturer;
	$data{lensmodel_id} = $href->{lensmodel_id} // &listchoices({table=>'choose_lensmodel', where=>{'manufacturer_id'=>$manufacturer_id}, required=>1, inserthandler=>\&lensmodel_add});
	...
	if ($manufacturer_id == 3 && $data{date_code}) {
		my $introduced = &lookupval({col=>'introduced', table=>'LENSMODEL', where=>{lensmodel_id=>$data{lensmodel_id}}});
		my $discontinued = &lookupval({col=>'discontinued', table=>'LENSMODEL', where=>{lensmodel_id=>$data{lensmodel_id}}});
	...
	$data{condition_id} = $href->{condition_id} // &listchoices({keyword=>'condition', cols=>['condition_id as id', 'name as opt'], table=>'`CONDITION`'});
	$data{condition} = &prompt({prompt=>'Condition description'});

	my $lens_id = &newrecord({data=>\%data, table=>'LENS'});
	return $lens_id;
}
...
sub lensmodel_add {
	...
	# Gather data from user
	my $datahr = &lensmodel_prompt;
	my %data = %$datahr;

	my $lensmodel_id = &newrecord({data=>\%data, table=>'LENSMODEL'});

	if (&prompt({default=>'yes', prompt=>'Add accessory compatibility for this lens model?', type=>'boolean'})) {
		&lensmodel_accessory({lensmodel_id=>$lensmodel_id});
	}

	if (&prompt({default=>'no', prompt=>'Add this lens model to a series?', type=>'boolean'})) {
		&lensmodel_series({lensmodel_id=>$lensmodel_id});
	}
	return $lensmodel_id;
}
...
sub lensmodel_edit {
	...
	# Gather data from user
	my $data = &lens_prompt({defaults=>$existing});
...
sub lensmodel_prompt {
	...
	if ($data{fixed_mount} == 0) {
		$data{mount_id} = &listchoices({cols=>['mount_id as id', 'mount as opt'], table=>'choose_mount', inserthandler=>\&mount_add, default=>$$defaults{mount_id}});
	...
	$data{negative_size_id} = &listchoices({cols=>['negative_size_id as id', 'negative_size as opt'], table=>'NEGATIVE_SIZE', inserthandler=>\&negativesize_add, default=>$$defaults{negative_size_id}});
...
sub lensmodel_accessory {
	my $href = shift;
	my $lensmodel_id = $href->{lensmodel_id} // &listchoices({table=>'choose_lensmodel', required=>1});
	while (1) {
		my %compatdata;
		$compatdata{accessory_id} = $href->{accessory_id} // &listchoices({table=>'choose_accessory'});
		$compatdata{lensmodel_id} = $lensmodel_id;
		&newrecord({data=>\%compatdata, table=>'ACCESSORY_COMPAT'});
		last if (!&prompt({default=>'yes', prompt=>'Add more accessory compatibility info?', type=>'boolean'}));
	}
	return;
}
...
sub lensmodel_series {
	my $href = shift;
	my %data;
	$data{lensmodel_id} = $href->{lensmodel_id} // &listchoices({table=>'choose_lensmodel', required=>1});
	$data{series_id} = $href->{series_id} // &listchoices({cols=>['series_id as id', 'name as opt'], table=>'SERIES', required=>1, inserthandler=>\&series_add});
	return &newrecord({data=>\%data, table=>'SERIES_MEMBER'});
}
...
sub lensmodel_info {
        ...
        # Choose lens
        my $lensmodel_id = $href->{lens_id} // &listchoices({table=>'choose_lensmodel', required=>1});

        # Get lens data
        my $lensdata = &lookupcol({table=>'info_lensmodel', where=>{'`Lens Model ID`'=>$lensmodel_id}});

        # Show compatible accessories
        my $accessories = &lookuplist({col=>'opt', table=>'choose_accessory_compat', where=>{lensmodel_id=>$lensmodel_id}});
        ${@$lensdata[0]}{'Accessories'} = $accessories;

        # Show compatible cameras
        my $cameras = &lookuplist({col=>'camera', table=>'cameralens_compat', where=>{lensmodel_id=>$lensmodel_id}});
...
sub lens_sell {
	...
	return &updaterecord({data=>\%data, table=>'LENS', where=>{lens_id=>$lens_id}});
}
...
sub lens_repair {
	my $href = shift;
	my %data;
	$data{lens_id} = $href->{lens_id} // &listchoices({table=>'choose_lens', required=>1});
...
sub lens_info {
	...
	# Get lens data
	my $lensdata = &lookupcol({table=>'info_lens', where=>{'`Lens ID`'=>$lens_id}});
	...
	# Show compatible cameras
	my $cameras = &lookuplist({col=>'camera', table=>'cameralens_compat', where=>{lens_id=>$lens_id}});
	${@$lensdata[0]}{'Cameras'} = $cameras;
	...
	my $maxaperture = &lookupval({col=>'max_aperture', table=>'LENSMODEL', where=>{lensmodel_id=>$lensmodel_id}});
	my $modeaperture = &lookupval({query=>"select aperture from NEGATIVE where aperture is not null and lens_id=$lens_id group by aperture order by count(aperture) desc limit 1"});
	print "\tThis lens has a maximum aperture of f/$maxaperture but you most commonly use it at f/$modeaperture\n";
	if (&lookupval({col=>'zoom', table=>'LENSMODEL', where=>{lensmodel_id=>$lensmodel_id}})) {
		my $minf = &lookupval({col=>'min_focal_length', table=>'LENSMODEL', where=>{lensmodel_id=>$lensmodel_id}});
		my $maxf = &lookupval({col=>'max_focal_length', table=>'LENSMODEL', where=>{lensmodel_id=>$lensmodel_id}});
		my $meanf = &lookupval({col=>'avg(focal_length)', table=>'NEGATIVE', where=>{lens_id=>$lens_id}});
...
sub print_add {
	my $href = shift;
	my %data;
	my $todo_id = $href->{todo_id} // &listchoices({keyword=>'print from the order queue', table=>'choose_todo'});
	if ($todo_id) {
		$data{negative_id} = $href->{negative_id} // &lookupval({col=>'negative_id', table=>'TO_PRINT', where=>{id=>$todo_id}});
	} else {
		$data{negative_id} = $href->{negative_id} // &chooseneg;
	}
	...
	$data{paper_stock_id} = $href->{paper_stock_id} // &listchoices({keyword=>'paper stock', table=>'choose_paper', inserthandler=>\&paperstock_add});
	...
	$data{enlarger_id} = $href->{enlarger_id} // &listchoices({table=>'choose_enlarger', inserthandler=>\&enlarger_add});
	$data{lens_id} = $href->{lens_id} // &listchoices({table=>'choose_enlarger_lens'});
	$data{developer_id} = $href->{developer_id} // &listchoices({cols=>['developer_id as id', 'name as opt'], table=>'DEVELOPER', where=>{'for_paper'=>1}, inserthandler=>\&developer_add});
	$data{printer_id} = $href->{printer_id} // &listchoices({keyword=>'printer', cols=>['person_id as id', 'name as opt'], table=>'PERSON', inserthandler=>\&person_add});
	my @prints;
	for my $n (1..$qty) {
		...
		$data{notes} = $href->{notes} // &prompt({prompt=>'Notes'});
		my $print_id = &newrecord({data=>\%data, table=>'PRINT'});
		push @prints, $print_id;

		&print_tone({print_id=>$print_id}) if (&prompt({default=>'no', prompt=>'Did you tone this print?', type=>'boolean'}));
		&print_archive({print_id=>$print_id}) if (&prompt({default=>'no', prompt=>'Archive this print?', type=>'boolean'}));
	}

	print "\nAdded $qty prints in this run, numbered #$prints[0]-$prints[-1]\n" if ($qty > 1);

	# Mark is as complete in the todo list
	&updaterecord({data=>{printed=>1, print_id=>$prints[-1]}, table=>'TO_PRINT', where=>{id=>$todo_id}}) if ($todo_id);
...
sub print_fulfil {
	my $href = shift;
	my %data;
	my $todo_id = $href->{todo_id} // &listchoices({keyword=>'print from the queue', table=>'choose_todo', required=>1});
	...
	return &updaterecord({data=>\%data, table=>'TO_PRINT', where=>{id=>$todo_id}});
}
...
sub print_tone {
	...
	$data{toner_id} = $href->{toner_id} // &listchoices({cols=>['toner_id as id', 'toner as opt'], table=>'TONER', inserthandler=>\&toner_add});
	my $dilution1 = &lookupval({col=>'stock_dilution', table=>'TONER', where=>{toner_id=>$data{toner_id}}});
	...
	if (&prompt({default=>'no', prompt=>'Did you use a second toner?', type=>'boolean'}) == 1) {
		$data{'2nd_toner_id'} = $href->{'2nd_toner_id'} // &listchoices({cols=>['toner_id as id', 'toner as opt'], table=>'TONER', inserthandler=>\&toner_add});
		my $dilution2 = &lookupval({col=>'stock_dilution', table=>'TONER', where=>{toner_id=>$data{'2nd_toner_id'}}});
	...
	return &updaterecord({data=>\%data, table=>'PRINT', where=>{print_id=>$print_id}});
}
...
sub print_sell {
	...
	&print_unarchive({print_id=>$print_id});
	return &updaterecord({data=>\%data, table=>'PRINT', where=>{print_id=>$print_id}});
}
...
sub print_order {
	...
	return &newrecord({data=>\%data, table=>'TO_PRINT'});
}
...
sub print_archive {
	...
	$data{archive_id} = $href->{archive_id} // &listchoices({cols=>['archive_id as id', 'name as opt'], table=>'ARCHIVE', where=>{'archive_type_id'=>3, 'sealed'=>0}, inserthandler=>\&archive_add, required=>1});
	$data{own} = 1;
...
sub print_unarchive {
	...
	return &call({procedure=>'print_unarchive', args=>[$print_id]});
}
...
sub print_locate {
	...
	if (my $archiveid = &lookupval({col=>'archive_id', table=>'PRINT', where=>{print_id=>$print_id}})) {
		my $archive = &lookupval({col=>"concat(name, ' (', location, ')') as archive", table=>'ARCHIVE', where=>{archive_id=>$archiveid}});
		print "Print #${print_id} is in $archive\n";
	} elsif (my $location = &lookupval({col=>'location', table=>'PRINT', where=>{print_id=>$print_id}})) {
		if (my $own = &lookupval({col=>'own', table=>'PRINT', where=>{print_id=>$print_id}})) {
...
sub print_info {
	...
	print Dump(&lookupcol({table=>'info_print', where=>{Print=>$print_id}}));
	return;
}
...
sub print_exhibit {
	...
	$data{exhibition_id} = &listchoices({cols=>['exhibition_id as id', 'title as opt'], table=>'EXHIBITION', inserthandler=>\&exhibition_add, required=>1});
	return &newrecord({data=>\%data, table=>'EXHIBIT'});
}
...
sub print_label {
	...
	my $data = &lookupcol({table=>'info_print', where=>{Print=>$print_id}});
	my $row = @$data[0];
...
sub print_worklist {
	my $href = shift;
	my $data = &lookupcol({table=>'choose_todo'});
...
sub paperstock_add {
	my $href = shift;
	my %data;
	$data{manufacturer_id} = $href->{manufacturer_id} // &choose_manufacturer;
	...
	return &newrecord({data=>\%data, table=>'PAPER_STOCK'});
}
...
sub developer_add {
	...
	return &newrecord({data=>\%data, table=>'DEVELOPER'});
}
...
sub mount_add {
	...
	return &newrecord({data=>\%data, table=>'MOUNT'});
}

# View compatible cameras and lenses for a mount
sub mount_info {
	my $href = shift;
	my $mountid = $href->{mount_id} // &listchoices({cols=>['mount_id as id', 'mount as opt'], table=>'choose_mount', required=>1});
	my $mount = &lookupval({col=>'mount', table=>'choose_mount', where=>{mount_id=>${mountid}}});
	print "Showing data for $mount mount\n";
	&printlist({msg=>"cameras with $mount mount", cols=>"distinct camera_id as id, camera as opt", table=>'cameralens_compat', where=>{mount_id=>$mountid}, order=>'opt'});
	&printlist({msg=>"lenses with $mount mount", cols=>"distinct lens_id as id, lens as opt", table=>'cameralens_compat', where=>{mount_id=>$mountid}, order=>'opt'});
	return;
}
...
sub toner_add {
	...
	return &newrecord({data=>\%data, table=>'TONER'});
}
...
sub filmstock_add {
	...
	$data{process_id} = $href->{process_id} // &listchoices({cols=>['process_id as id', 'name as opt'], table=>'PROCESS', inserthandler=>\&process_add});
	return &newrecord({data=>\%data, table=>'FILMSTOCK'});
}
...
sub teleconverter_add {
	...
	$data{mount_id} = $href->{mount_id} // &listchoices({cols=>['mount_id as id', 'mount as opt'], table=>'choose_mount', where=>{'purpose'=>'Camera'}, inserthandler=>\&mount_add});
	...
	return &newrecord({data=>\%data, table=>'TELECONVERTER'});
}
...
sub filter_add {
	...
	return &newrecord({data=>\%data, table=>'FILTER'});
}
...
sub process_add {
	...
	return &newrecord({data=>\%data, table=>'PROCESS'});
}
...
sub filter_adapt {
	...
	return &newrecord({data=>\%data, table=>'FILTER_ADAPTER'});
}
...
sub manufacturer_add {
	...
	return &newrecord({data=>\%data, table=>'MANUFACTURER'});
}
...
sub accessory_add {
	my $href = shift;
	my %data;
	$data{accessory_type_id} = $href->{accessory_type_id} // &listchoices({cols=>['accessory_type_id as id', 'accessory_type as opt'], table=>'ACCESSORY_TYPE', inserthandler=>\&accessory_type});
	$data{manufacturer_id} = $href->{manufacturer_id} // &choose_manufacturer;
	...
	my $accessoryid = &newrecord({data=>\%data, table=>'ACCESSORY'});
	...
	if (&prompt({default=>'yes', prompt=>'Add camera compatibility info for this accessory?', type=>'boolean'})) {
		while (1) {
			...
			$compatdata{camera_id} = &listchoices({table=>'choose_camera', required=>1});
			&newrecord({data=>\%compatdata, table=>'ACCESSORY_COMPAT', silent=>1});
	...
	if (&prompt({default=>'yes', prompt=>'Add lens compatibility info for this accessory?', type=>'boolean'})) {
		while (1) {
			my %compatdata;
			$compatdata{accessory_id} = $accessoryid;
			$compatdata{lens_id} = &listchoices({table=>'choose_lens', required=>1});
			&newrecord({data=>\%compatdata, table=>'ACCESSORY_COMPAT', silent=>1});
...
sub accessory_category {
	...
	return &newrecord({data=>\%data, table=>'ACCESSORY_TYPE'});
}

# Display info about an accessory
sub accessory_info {
	my $href = shift;
	my $accessory_id = $href->{accessory_id} // &listchoices({table=>'choose_accessory'});
	print Dump(&lookupcol({table=>'info_accessory', where=>{'`Accessory ID`'=>$accessory_id}}));
	return;
}
...
sub enlarger_add {
	...
	$data{negative_size_id} = $href->{negative_size_id} // &listchoices({cols=>['negative_size_id as id', 'negative_size as opt'], table=>'NEGATIVE_SIZE', inserthandler=>\&negativesize_add});
	...
	return &newrecord({data=>\%data, table=>'ENLARGER'});
}

# Display info about an enlarger
sub enlarger_info {
	my $href = shift;
	my $enlarger_id = $href->{enlarger_id} // &listchoices({table=>'choose_enlarger', required=>1});
	print Dump(&lookupcol({table=>'info_enlarger', where=>{'`Enlarger ID`'=>$enlarger_id}}));
	return;
}
...
sub enlarger_sell {
	...
	return &updaterecord({data=>\%data, table=>'ENLARGER', where=>{enlarger_id=>$enlarger_id}});
}
...
sub flash_add {
	...
	if ($data{battery_powered} == 1) {
		$data{battery_type_id} = $href->{battery_type_id} // &listchoices({keyword=>'battery type', table=>'choose_battery', inserthandler=>\&battery_add});
	...
	if ($data{ttl} == 1) {
		$data{flash_protocol_id} = $href->{flash_protocol_id} // &listchoices({table=>'choose_flash_protocol'});
	}
	...
	return &newrecord({data=>\%data, table=>'FLASH'});
}
...
sub battery_add {
	...
	return &newrecord({data=>\%data, table=>'BATTERY'});
}
...
sub format_add {
	...
	return &newrecord({data=>\%data, table=>'FORMAT'});
}
...
sub negativesize_add {
	...
	return &newrecord({data=>\%data, table=>'NEGATIVE_SIZE'});
}
...
sub mount_adapt {
	my $href = shift;
	my %data;
	$data{lens_mount} = $href->{lens_mount} // &listchoices({keyword=>'lens-facing mount', cols=>['mount_id as id', 'mount as opt'], table=>'choose_mount', where=>{'purpose'=>'Camera'}, inserthandler=>\&mount_add});
	$data{camera_mount} = $href->{camera_mount} // &listchoices({keyword=>'camera-facing mount', cols=>['mount_id as id', 'mount as opt'], table=>'choose_mount', where=>{'purpose'=>'Camera'}, inserthandler=>\&mount_add});
	...
	$data{notes} = $href->{notes} // &prompt({prompt=>'Notes'});
	return &newrecord({data=>\%data, table=>'MOUNT_ADAPTER'});
}
...
sub lightmeter_add {
	...
	$data{metering_type} = $href->{metering_type} // &listchoices({cols=>['metering_type_id as id', 'metering as opt'], table=>'METERING_TYPE', inserthandler=>\&meteringtype_add});
	...
	return &newrecord({data=>\%data, table=>'LIGHT_METER'});
}
...
sub camera_addbodytype {
	...
	return &newrecord({data=>\%data, table=>'BODY_TYPE'});
}
...
sub series_add {
	...
	return &newrecord({data=>\%data, table=>'SERIES'});
}

# Print info about a series
sub series_info {
	my $href = shift;
	my $series_id = $href->{series_id} // &listchoices({cols=>['series_id as id', 'name as opt'], table=>'SERIES', required=>1});
	my $seriesname = &lookupval({col=>'name', table=>'SERIES', where=>{series_id=>$series_id}});
	my $total = &printlist({msg=>"camera and lens models in series '$seriesname'", table=>'info_series', cols=>["Got as id", 'Model as opt'], where=>{'`Series ID`'=>$series_id}});
	my $got = &lookupval({col=>'count(*)', table=>'info_series', where=>{'`Series ID`'=>$series_id, Got=>'✓'}});
...
sub series_list {
	my $href = shift;
	my $rows = &tabulate({view=>'summary_series'});
	return $rows;
}
...
sub series_need {
	my $href = shift;
	my $rows = &printlist({msg=>'models needed to complete series', cols=>["'' as id", 'Model as opt'], table=>'info_series', where=>{'Got'=>'✗'}});
	return $rows;
}
...
sub archive_add {
	my $href = shift;
	my %data;
	$data{archive_type_id} = $href->{archive_type_id} // &listchoices({cols=>['archive_type_id as id', 'archive_type as opt'], table=>'ARCHIVE_TYPE'});
	...
	return &newrecord({data=>\%data, table=>'ARCHIVE'});
}
...
sub archive_films {
	...
	$data{archive_id} = $href->{archive_id} // &listchoices({cols=>['archive_id as id', 'name as opt'], table=>'ARCHIVE', where=>'archive_type_id in (1,2) and sealed = 0', inserthandler=>\&archive_add});
	return &updaterecord({data=>\%data, table=>'FILM', where=>"film_id >= $minfilm and film_id <= $maxfilm and archive_id is null"});
}

# Display info about an archive
sub archive_info {
	my $href = shift;
	my $archive_id = $href->{archive_id} // &listchoices({cols=>['archive_id as id', 'name as opt'], table=>'ARCHIVE', required=>1});
	print Dump(&lookupcol({table=>'info_archive', where=>{'`Archive ID`'=>$archive_id}}));
	return;
}
...
sub archive_list {
	my $href = shift;
	my $archive_id = $href->{archive_id} // &listchoices({cols=>['archive_id as id', 'name as opt'], table=>'ARCHIVE', required=>1});
	my $archive_name = &lookupval({col=>'name', table=>'ARCHIVE', where=>{archive_id=>$archive_id}});
	&printlist({msg=>"items in archive $archive_name", table=>'archive_contents', where=>{archive_id=>$archive_id}});
	return;
}
...
sub archive_seal {
	my $href = shift;
	my %data;
	my $archive_id = $href->{archive_id} // &listchoices({cols=>['archive_id as id', 'name as opt'], table=>'ARCHIVE', where=>{sealed=>0}, required=>1});
	$data{sealed} = 1;
	return &updaterecord({data=>\%data, table=>'ARCHIVE', where=>{archive_id=>$archive_id}});
}
...
sub archive_unseal {
	my $href = shift;
	my %data;
	my $archive_id = $href->{archive_id} // &listchoices({cols=>['archive_id as id', 'name as opt'], table=>'ARCHIVE', where=>{sealed=>1}, required=>1});
	$data{sealed} = 0;
	return &updaterecord({data=>\%data, table=>'ARCHIVE', where=>{archive_id=>$archive_id}});
}
...
sub archive_move {
	...
	my %data;
	my $archive_id = $href->{archive_id} // &listchoices({cols=>['archive_id as id', 'name as opt'], table=>'ARCHIVE', required=>1});
	my $oldlocation = &lookupval({col=>'location', table=>'ARCHIVE', where=>{archive_id=>$archive_id}});
...
sub shuttertype_add {
	...
	return &newrecord({data=>\%data, table=>'SHUTTER_TYPE'});
}
...
sub focustype_add {
	...
	return &newrecord({data=>\%data, table=>'FOCUS_TYPE'});
}
...
sub flashprotocol_add {
	...
	return &newrecord({data=>\%data, table=>'FLASH_PROTOCOL'});
}
...
sub meteringtype_add {
	...
	return &newrecord({data=>\%data, table=>'METERING_TYPE'});
}
...
sub shutterspeed_add {
	...
	return &newrecord({data=>\%data, table=>'SHUTTER_SPEED'});
}
...
sub person_add {
	...
	return &newrecord({data=>\%data, table=>'PERSON'});
}
...
sub projector_add {
	...
	$data{mount_id} = $href->{mount_id} // &listchoices({cols=>['mount_id as id', 'mount as opt'], table=>'choose_mount', where=>{'purpose'=>'Projector'}, inserthandler=>\&mount_add});
	$data{negative_size_id} = $href->{negative_size_id} // &listchoices({cols=>['negative_size_id as id', 'negative_size as opt'], table=>'NEGATIVE_SIZE', inserthandler=>\&negativesize_add});
	$data{own} = 1;
	$data{cine} = $href->{cine} // &prompt({prompt=>'Is this a cine/movie projector?', type=>'boolean'});
	return &newrecord({data=>\%data, table=>'PROJECTOR'});
}
...
sub movie_add {
	...
	$data{camera_id} = $href->{camera_id} // &listchoices({table=>'choose_movie_camera'});
	if (&lookupval({col=>'fixed_mount', table=>'CAMERA', where=>{camera_id=>$data{camera_id}}})) {
		$data{lens_id} = &lookupval({col=>'lens_id', table=>'CAMERA', where=>{camera_id=>$data{camera_id}}});
	} else {
		$data{lens_id} = $href->{lens_id} // &listchoices({table=>'choose_lens'});
	}
	$data{format_id} = $href->{format_id} // &listchoices({cols=>['format_id as id', 'format as opt'], table=>'FORMAT', inserthandler=>\&format_add});
	...
	$data{filmstock_id} = $href->{filmstock_id} // &listchoices({table=>'choose_filmstock', inserthandler=>\&filmstock_add});
	...
	$data{process_id} = $href->{process_id} // &listchoices({keyword=>'process', cols=>['process_id as id', 'name as opt'], table=>'PROCESS', inserthandler=>\&process_add});
	$data{description} = $href->{description} // &prompt({prompt=>'Please enter a description of the movie'});
	return &newrecord({data=>\%data, table=>'MOVIE'});
}

# Show info about a movie
sub movie_info {
	my $href = shift;
	my $movie_id = $href->{movie_id} // &listchoices({cols=>['movie_id as id', 'title as opt'], table=>'MOVIE', required=>1});
	print Dump(&lookupcol({table=>'info_movie', where=>{'`Movie ID`'=>$movie_id}}));
	return;
}
...
sub audit_shutterspeeds {
	my $href = shift;
	my $cameramodel_id = $href->{cameramodel_id} // &listchoices({keyword=>'camera model without shutter speed data', table=>'choose_cameramodel_without_shutter_data', required=>1});
	&cameramodel_shutterspeeds({cameramodel_id=>$cameramodel_id});
	return;
}
...
sub audit_exposureprograms {
	my $href = shift;
	my $cameramodel_id = $href->{cameramodel_id} // &listchoices({keyword=>'camera model without exposure program data', table=>'choose_cameramodel_without_exposure_programs', required=>1});
	&cameramodel_exposureprogram({cameramodel_id=>$cameramodel_id});
	return;
}
...
sub audit_meteringmodes {
	my $href = shift;
	my $cameramodel_id = $href->{cameramodel_id} // &listchoices({keyword=>'camera without metering mode data', table=>'choose_cameramodel_without_metering_data', required=>1});
	&cameramodel_meteringmode({cameramodel_id=>$cameramodel_id});
	return;
}
...
sub exhibition_add {
	...
	return &newrecord({data=>\%data, table=>'EXHIBITION'});
}

# Review which prints were exhibited at an exhibition
sub exhibition_info {
	my $href = shift;
	my $exhibition_id = &listchoices({cols=>['exhibition_id as id', 'title as opt'], table=>'EXHIBITION', required=>1});
	my $title = &lookupval({col=>'title', table=>'EXHIBITION', where=>{exhibition_id=>$exhibition_id}});

	&printlist({msg=>"prints exhibited at $title", table=>'exhibits', where=>{exhibition_id=>$exhibition_id}});
	return;
}
...
sub run_task {
	...
	if (defined($action) && $choices[$action]->{proc}) {
		my $rows = &call({procedure=>$choices[$action]->{proc}});
		$rows = &unsci($rows);
...
sub run_report {
	...
	if (defined($action) && $choices[$action]->{view}) {
		&tabulate({view=>$choices[$action]->{view}});
	}
	return;
}
...
sub choose_manufacturer {
	...
	if ($default) {
		my $manufacturer = &lookupval({col=>'manufacturer', table=>'MANUFACTURER', where=>{manufacturer_id=>$default}});
	...
	$initial = lc($initial);
	return &listchoices({cols=>['manufacturer_id as id', 'manufacturer as opt'], table=>'MANUFACTURER', where=>{'lower(left(manufacturer, 1))'=>$initial}, inserthandler=>\&handlers::manufacturer_add, required=>1});
}

# Audit cameras without display lenses set
sub audit_displaylenses {
	my $href = shift;
	my $camera_id = $href->{camera_id} // &listchoices({keyword=>'camera', table=>'camera_chooser', where=>{mount_id=>{'!=', undef}, display_lens=>{'=', undef}}, required=>1 });
	&camera_displaylens({camera_id=>$camera_id});
	return;
}
...
sub db_stats {
	my $href = shift;
	my %data;
	$data{'Total cameras'} = &lookupval({col=>'count(camera_id)', table=>'CAMERA'});
	$data{'Total lenses'} = &lookupval({col=>'count(lens_id)', table=>'LENS'});
	$data{'Total negatives'} = &lookupval({col=>'count(negative_id)', table=>'NEGATIVE'});
	$data{'Total prints'} = &lookupval({col=>'count(print_id)', table=>'PRINT'});
	print Dump(\%data);
	return;
}
...
sub db_logs {
	my $href = shift;
	print Dump(&lookuplist({col=>"concat(datetime, ' ', type, ' ', message) as log", table=>'LOG'}));
	return;
}
...
sub scan_add {
	...
	return &newrecord({data=>\%data, table=>'SCAN'});
}
...
sub scan_edit {
	...
	my $scan_id = $href->{scan_id} // &choosescan;

	# Work out negative_id or print_id
	my $scan_data = &lookupcol({cols=>['negative_id', 'print_id'], table=>'SCAN', where=>{scan_id=>$scan_id}});
	$scan_data = &thin($$scan_data[0]);

	# Insert new scan from same source
	return &scan_add({scan_data=>$scan_data});
}
...
sub scan_delete {
	...
	my $relativepath = &lookupval({col=>"concat(directory, '/', filename)", table=>'scans_negs', where=>{scan_id=>$scan_id}});
	...
	return &deleterecord({table=>'SCAN', where=>{scan_id=>$scan_id}});
}
			...
			if ($auto || &prompt({prompt=>"Add $fsonlyfile to the database?", type=>'boolean', required=>1})) {
				...
				if ($filename =~ m/^(\d+)-([0-9a-z]+)-.+\.jpg$/i) {
					...
					if ($auto || &prompt({prompt=>"This looks like a scan of negative $film_id/$frame. Add it?", type=>'boolean', default=>'yes', required=>1})) {
						...
						my $subdir = &lookupval({col=>'directory', table=>'FILM', where=>{film_id=>$film_id}});
						...
						&newrecord({data=>{negative_id=>$neg_id, filename=>$filename}, table=>'SCAN', silent=>$auto});
				...
				# Test to see if this is from a print, e.g. P232-image012.jpg
					...
					if ($auto || &prompt({prompt=>"This looks like a scan of print #$print_id. Add it?", type=>'boolean', default=>'yes', required=>1})) {
						&newrecord({data=>{print_id=>$print_id, filename=>$filename}, table=>'SCAN', silent=>$auto});
	...
	if ($numdbonly>0 && &prompt({prompt=>"Audit $numdbonly scans that exist only in the database and not on the filesystem?", type=>'boolean', default=>'no', required=>1})) {
		...
		for my $dbonlyfile (@dbonly) {
			if (&prompt({prompt=>"Delete $dbonlyfile from the database?", type=>'boolean', default=>'no', required=>1})) {
				...
				&deleterecord({table=>'SCAN', where=>{filename=>$filename}, silent=>1});
				$x++;
			}
		}
		...
		if (defined($ref->{'filename'})) {
			...
			if (-e "$basepath/$ref->{'directory'}/$ref->{'filename'}") {
				...
				if ($ref->{'filename'} ne $newname) {
					...
					# Update scan in db
					&updaterecord({data=>{filename=>$newname}, table=>'SCAN', where=>{scan_id=>$ref->{scan_id}}, silent=>1});
				}
			}
		}
	}
