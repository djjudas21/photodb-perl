...
This guide tries to explain the key concepts behind PhotoDB. It is not an exhaustive guide to every command available. To list all commands, run C<photodb help>.
To list all subcommands available under a command, run (for example) C<photodb camera>. Each subcommand has a brief explanation with it.
...
The command C<photodb data> has subcommands for adding various pieces of data. Normally, you shouldn't need to go out of your way to add this type of data, as
...
You will be guided through questions when adding a new camera or lens by running C<photodb camera add> or C<photodb lens add>. When adding a fixed-lens camera
...
you only specify the lens mount. You can add lenses separately by running C<photodb lens add>, which are then available to use with any camera with the same
mount.
...
PhotoDB lets you record a stash of films by running C<photodb film add>, which you can then load into a camera with C<photodb film load>. Films are associated
with a camera. They can be developed with C<photodb film develop> and archived with C<photodb film archive>.
...
are added by running C<photodb negative add>.
...
from. You'll be able to add other info about how the print was made. Prints are added by running C<photodb print add>.

You can also record orders for prints with C<photodb print order>, view your printing to-do list with C<photodb print worklist> and record sales of prints with
<photodb print sell>.
...
more than once. Each scan must be recorded separately. You can run C<photodb scan add> to manually add a scan to the database.
...
to search for and add scans automatically by running C<photodb scan search>.
...
Use C<photodb negative tag> to tag all scans of a single negative or C<photodb film tag> to tag all scans from an entire film.
...
All of the above "special" accessories can be added to the database with C<photodb accessory <accessory>>.
...
Add new general accessories with C<photodb accessory add> and add new categories of general accessories with C<photodb accessory category>.
